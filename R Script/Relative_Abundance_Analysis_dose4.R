...
sample_data(ps_4)$Response <- factor(
  sample_data(ps_4)$Response,
  levels = c("None", "IgA", "IgG", "Both")
)
contrasts(sample_data(ps_4)$Response) <- contr.treatment(
  n = 4,          # number of levels
  base = 1        # reference = level 1 = "None"
)
contrasts(sample_data(ps_4)$Response)
ps_4
#ancom dose 4
...
#Volcano plot
Volcanoplot_dose4 <- ggplot(volcano_df, aes(x = lfc_ResponseBoth, 
  ...
  labs(
    title = "Volcano Plot – Response with 4-doses",
...
Sig_taxa_dose4 <- ggplot(sig_taxa, 
  ...
  labs(
    title = "Significant Taxa - Response with 4-doses",
    x = "Genus",
...
#saving plot
ggsave("Volcano_plot_dose4.png", plot = Volcanoplot_dose4,
       width = 8,
       height = 6,
       dpi = 300)

ggsave("Sig_taxa_plot_dose4.png", plot = Sig_taxa_dose4,
       width = 8,
       height = 6,
       dpi = 300)
